---
sidebar_position: 21
slug: /plugin-options-entryFilter
title: entryFilter
tags: [template, entry, entryFilter]
---
...
The option applies only when the [`entry`](/plugin-options-entry) is configured as a reference to the templates directory.

## Default Behavior

The default `includes` property varies based on the configured [preprocessor](/plugin-options-preprocessor).
