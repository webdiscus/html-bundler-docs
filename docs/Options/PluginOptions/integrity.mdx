...
>
> - The [`js.filename`](/plugin-options-js) and [`css.filename`](/plugin-options-css) options must contain the `contenthash`.
...
See the [`integrityHashes`](/hooks-and-callbacks#integrityhashes) hook.
