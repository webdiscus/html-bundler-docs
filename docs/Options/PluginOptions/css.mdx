...
- `filename` - an output filename of extracted CSS. Details see by [filename option](/plugin-options-filename).
- `chunkFilename` - an output filename of non-initial chunk files, e.g., a style file imported in JavaScript.
- `outputPath` - an output path of extracted CSS. Details see by [outputPath option](/plugin-options-outputPath).
...
If you use the [Live Reload](/configuration/live-reload) configuration, then be sure to exclude the style files (CSS/SCSS) from watching,
